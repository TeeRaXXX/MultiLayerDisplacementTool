# settings.py — property groups for Multi Layer Displacement Tool
...
# ---- alias getters/setters (for UI backward-compat) --------------------------

def _get_is_painting(self): return bool(getattr(self, "painting", False))
...
def _get_subdiv_viewport_levels(self): return int(getattr(self, "subdiv_view", 1))
def _set_subdiv_viewport_levels(self, v): self["subdiv_view"] = int(v)

def _get_subdiv_render_levels(self): return int(getattr(self, "subdiv_render", 1))
def _set_subdiv_render_levels(self, v): self["subdiv_render"] = int(v)

def _get_fill_empty_vc_channels_with_white(self): return bool(getattr(self, "fill_empty_vc_white", False))
def _set_fill_empty_vc_channels_with_white(self, v): self["fill_empty_vc_white"] = bool(v)

# новые алиасы под текущий ui.py:
def _get_refine_enable(self): return bool(getattr(self, "subdiv_enable", False))
def _set_refine_enable(self, v): self["subdiv_enable"] = bool(v)

def _get_subdiv_levels_view(self): return int(getattr(self, "subdiv_view", 1))
def _set_subdiv_levels_view(self, v): self["subdiv_view"] = int(v)

def _get_subdiv_levels_render(self): return int(getattr(self, "subdiv_render", 1))
def _set_subdiv_levels_render(self, v): self["subdiv_render"] = int(v)

def _get_preview_blend(self): return bool(getattr(self, "preview_enable", False))
def _set_preview_blend(self, v): self["preview_enable"] = bool(v)

def _get_mat_assign_threshold(self): return float(getattr(self, "mask_threshold", 0.05))
def _set_mat_assign_threshold(self, v): self["mask_threshold"] = float(v)
...
class MLD_Settings(PropertyGroup):
    ...
    subdiv_enable: BoolProperty(
        ...
        description="Enable Subdivision modifier (preview helper)",
    )
    ...
    preview_enable: BoolProperty(
        ...
        description="Build and assign a HeightLerp-like preview material for the object",
    ...
    decimate_enable: BoolProperty(
        ...
        description="Enable decimate for preview (applied after GN in the pipeline)",
    )
    ...
    # ---------------- Aliases for backward compatibility (used by UI) ----------
